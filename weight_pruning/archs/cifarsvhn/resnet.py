...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out += self.shortcut(x)
        if(self.pruning):
            out = F.relu(out - self.v_threshold)
        else:
            out = self.lif2(out)
            spike_list[time].append(out.detach())
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, total_timestep =6, pruning=False, local=False, v_threshold=1.0):
        ...
        self.lif_input = newLIFNode(v_threshold=v_threshold, v_reset=0.0, tau=tau_global,
                                        surrogate_function=surrogate.ATan(),
                                        detach_reset=True)
        ...
        self.lif_fc = newLIFNode(v_threshold=v_threshold, v_reset=0.0, tau= tau_global,
                                        surrogate_function=surrogate.ATan(),
                                        detach_reset=True)
    ...
    def forward(self, x):
        ...
        for t in range(self.total_timestep):
            time = t
            if(self.pruning):
                out = F.relu(static_x - self.v_threshold)
            else:
                out = self.lif_input(static_x)
                spike_list[t].append(out.detach())
                v_list[t].append(self.lif_input.v_keep)
            ...
            spike_list[t].append(out.detach())
            v_list[t].append(self.lif_fc.v_keep)
